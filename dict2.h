...
void Goiy(GtkWidget *widget,gpointer data)
{
  ...
  store=GTK_LIST_STORE(gtk_tree_view_get_model(GTK_TREE_VIEW(DICT->listview)));//Du lieu cua list

  //printf("%s\n",word);
  strcpy(tu,word);
  s_tu[0]=tu[0]; s_tu[1]='\0';//Lay chu cai dau

  gtk_list_store_clear(store);//Xoa du lieu list
  ...
  //if (btsel(DICT->dict,tu,nghia,maxn,&realsize)!=0)
	...
	if (btsel(DICT->dict,s_tu,nghia,maxn,&realsize)==0)//Tim tu co chu cai dau == key soundex
    {
	    while(btseln(DICT->dict,tu1,nghia,maxn,&realsize)==0)
        {
		  SoundEx(soundex_key1,tu1,0,0);
		  if (strcmp(soundex_key,soundex_key1)==0) //so sanh 2 key soundex
...
void Thaydoi(GtkWidget *widget,gpointer data)
{
  ...
  if (gtk_tree_selection_get_selected(GTK_TREE_SELECTION(widget), &model, &iter))
    //Lay string ra tu o duoc chon trong list duoc tham chieu boi iter
  ...
  gtk_text_buffer_set_text(DICT->buffer,nghia,-1);//-1 danh doan text ket thuc = NULL
  g_free(tu);
}
...
gboolean Hoanthanh(GtkWidget *widget,GdkEventKey *event,gpointer data)
{
  ...
  if (btsel(DICT->dict,word1,nghia,maxn,&realsize)!=0) //Neu ko co
    {
